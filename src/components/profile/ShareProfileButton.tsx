...
export default function ShareProfileButton({
  ...
  const shareText = showAccuracy && accuracyScore !== null
    ? `@${username} on GRZLY — ${accuracyScore.toFixed(0)}% accuracy on short calls. Not financial advice.`
    : `@${username} on GRZLY — collective short conviction. Not financial advice.`
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute top-[calc(100%+6px)] left-0 z-40 bg-surface border border-border-hl rounded-[10px] p-1.5 shadow-2xl w-[210px]" onClick={e => e.stopPropagation()}>
          ...
          {/* Copy link */}
          <button
            onClick={() => copyText(profileUrl, 'link')}
